...
import ru.petcollector.petcollector.model.user.TelegramUser;
...
public interface UserMapper {
    ...
    public static User map(@NotNull final UserDTO userDTO, @NotNull final User user) {
        ...
        if (user.getTelegramUser() == null)
            user.setTelegramUser(new TelegramUser());

        user.getTelegramUser().setTelegramUserName(userDTO.getTelegramUserName().orElse(user.getTelegramUser().getTelegramUserName()));
        user.getTelegramUser().setUserTelegramId(userDTO.getUserTelegramId().orElse(user.getTelegramUser().getUserTelegramId()));
        user.getTelegramUser().setChatId(userDTO.getChatId().orElse(user.getTelegramUser().getChatId()));
        return user;
    }
